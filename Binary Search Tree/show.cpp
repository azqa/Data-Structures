...
//inserts the nodes
Drone * Show::insertHelper(const Drone& element, Drone *aDrone) {
    if(aDrone == nullptr){
        ...
        aDrone->m_left = insertHelper(element, aDrone->m_left); 
        //otherwise to the right
    } else if(element.m_id > aDrone->m_id) {
        aDrone->m_right = insertHelper(element, aDrone->m_right);
    } 
    //hight is updated
    updateHeight(aDrone);
...
//passes in a Drone pointer to clear the tree
...
Drone* Show::removeHelper(Drone *aDrone, int id){
    ...
    if(aDrone != nullptr) {
        ...
        //if being inserted is greater than then add it to the right                                              
                                                                                                                  
        if(id > aDrone->m_id) {
            aDrone->setRight(removeHelper(aDrone->m_right, id));
        }
               //if the id being inserted is less than then add to the left
	else if(id < aDrone->m_id) {
            aDrone->setLeft(removeHelper(aDrone->m_left, id));
        }
	//left child doesnt exsit
	 else if (aDrone->m_id == id && aDrone->m_left == nullptr) {
            //ids should match for aDrone to be null                                                                                                            
	...
	//right child doesnt exist
	 else if(aDrone->m_id == id && aDrone->m_right == nullptr){
	   if(m_root->m_id == aDrone->m_id) {
                m_root = nullptr;
            }
            Drone* temp = aDrone;
        ...
        //two children exist and the ids match
...
void Show::listDroneHelper(Drone* myDrone) const{
    ...
    if(myDrone != nullptr) {
        //uses in order traversalt to list the drones 
...
bool Show::setState(int id, STATE state){
    //calls the find function to find the drone with the id
    Drone *newDrone = findHelper(m_root, id);
    //if its exists then it sets the state
    if(newDrone != nullptr) {
